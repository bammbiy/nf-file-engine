...
5. `미리보기`에서 `ready`, `conflict`, `error` 상태를 검수합니다.
6. 승인/공유가 필요하면 `CSV 저장`으로 변경 예정 목록을 남깁니다.
7. 문제가 없으면 `변경 실행`을 누릅니다.
8. 실수한 경우 `마지막 배치 취소`로 직전 작업 묶음을 되돌립니다.
...
- `storage`: SQLite 히스토리 저장소 및 배치 단위 이력
- `ui`: Tkinter 데스크톱 화면
